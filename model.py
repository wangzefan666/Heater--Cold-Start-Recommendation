...
class Heater:
    ...
    def build_predictor(self, recall_at):
        self.eval_trainR = tf.sparse_placeholder(dtype=tf.float32, shape=[None, None], name='trainR_sparse')
    ...
    def get_eval_dict(self, _i, _eval_start, _eval_finish, eval_data):
        ...
        if self.phi_v_dim > 0:
            # zero_index 几乎没有好吧，不是要测试冷启动吗，合着都用了 preference 啊，没有 content 啥事啊
            # zero_index = np.where(np.sum(eval_data.V_pref_test, axis=1) == 0)[0]
            # dropout_item_indicator = np.ones((len(eval_data.test_item_ids), 1))
        ...
        if self.phi_u_dim > 0:
            ...
            # dropout_user_indicator = np.ones((_eval_finish - _eval_start, 1))
